...
def test_flash_attention(b, mean_sq, mean_sk, varlen, h, h_k, d, dv, causal, window, has_bwd, check_correctness: bool = True):
    ...
    if check_correctness:
        ...
        assert check_is_allclose("out", out_flash.detach().float(), out_torch.detach(), abs_tol=1e-3, rel_tol=8.01 / 128, cos_diff_tol=7e-6)
        assert check_is_allclose("lse", lse_flash.detach().float(), lse_torch.detach(), abs_tol=1e-6, rel_tol=2.01 / 65536)
        if has_bwd:
