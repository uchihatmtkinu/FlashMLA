...
// How the prefill outer loop obtains its query rows.
//   kClcGrid      the stock [2*s_q] problem-sized grid, rows handed out by
//                 Cluster Launch Control.
//   kPairWaveGrid a grid capped at the caller's SM budget, where each 2-CTA
//                 cluster claims two *adjacent* rows per wave from a closed
//                 form. Adjacent prefill tokens select overlapping TopK
//                 indices, so back-to-back rows reuse the same KV lines, and
//                 the CLC query/response traffic disappears entirely.
// Namespace scope, because the __global__ launcher templates on it too.
enum : int { kClcGrid = 0, kPairWaveGrid = 1 };
...
static constexpr bool IS_READY_DECODE = is_ready_decode_v<FWD_MODE>;
...
struct SharedMemoryPlan {
    ...
    transac_bar_t bar_q_tma_remote_ready;
    transac_bar_t bar_q_transform_remote_done;
};
...
template<int GRID_SCHEDULE = kClcGrid>
static __device__ void
sparse_attn_fwd_kernel_devfunc(
    const ArgT &params,
    const TmaParams &tma_params,
    int logical_block_x = -1,
    int logical_block_y = -1,
    // Number of extra, non-participating CTAs placed at the front of the grid
    // (a fused producer, for example). Cluster Launch Control hands out ids
    // over the whole grid, so those ids must be mapped back into this
    // kernel's own [0, 2*s_q) domain. Zero keeps the stock behaviour exactly.
    int clc_x_offset = 0
);

static TmaParams make_tma_params(const ArgT& params);
